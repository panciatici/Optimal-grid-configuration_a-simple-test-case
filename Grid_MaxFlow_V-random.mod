...
param N_Break {BUS}; #Number of Breakers by BUSBAR

param SubStation {BUS} symbolic; #Zone Index

param BIGM = max{k in BUS} abs(Pload[k]-Pgen[k])*10;
...
param B{(k,m) in N_X} := if ((k,m) in BRANCH) then 1/Branch_X[k,m] else 1/Branch_X[m,k];
...
# each busbar must be connected to at least one other busbar  
# for substation with more than one busbar
subject to Connected{k in BUS: N_Break[k]>=1}:
sum{(k,m)in BREAKER} Breaker_S[k,m] + sum{(m,k)in BREAKER} Breaker_S[m,k] >= 1;
